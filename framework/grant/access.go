...
func (a *Access) ProvidersForModel(model string) []schemas.ProviderCandidate {
	...
	// Providers a candidate was offered for, so the completion below tops up only what nothing here
	// could serve. Marked on emission rather than on sight: a provider permit that refuses this
	// model has not answered for the provider, and read as one permit another may still grant it.
	...
	for _, provider := range a.providersGrantedByAllowAll(offered, model) {
		// Nothing here could serve it, so it carries no routing preference and no key restriction: a
		// weight is something a provider permit expresses, and a provider none narrowed may use any
		// of its keys.
		candidates = append(candidates, schemas.ProviderCandidate{
			Provider: provider,
...
func (a *Access) providersGrantedByAllowAll(seen map[string]struct{}, model string) []string {
	if a.providers == nil || !a.grantsUnnamedProviders() {
		return nil
	}
	var granted []string
...
// grantsUnnamedProviders reports whether any permit grants providers it holds no provider permit
// for. Only those permits can put a provider into the completion above, so without one the walk can
// only answer with providers the access refuses — and it is the request path's hot loop, so it is
// worth not taking. Every provider a permit does name is enumerated before the completion runs, so
// nothing grantable is lost by skipping it: composing two refusals is a refusal under either mode.
func (a *Access) grantsUnnamedProviders() bool {
	for _, base := range a.bases {
		if base.AllowsAllProviders() {
			return true
		}
	}
	return a.scoping != nil && a.scoping.AllowsAllProviders()
}
