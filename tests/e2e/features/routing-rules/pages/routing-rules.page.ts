...
export interface RoutingRuleConfig {
  ...
  fallbacks?: Array<string | { provider: string; model?: string; key_id?: string }>
}
  ...
  async createRoutingRule(config: RoutingRuleConfig): Promise<void> {
    ...
    for (const fallback of config.fallbacks ?? []) {
      const spec = typeof fallback === 'string'
        ? { provider: fallback.split('/')[0], model: fallback.split('/').slice(1).join('/') || undefined, key_id: undefined }
        : fallback
      const index = await this.addFallbackProvider(spec.provider, spec.model)
      if (spec.key_id) {
        await this.pinFallbackKey(index, spec.key_id)
      }
    }
