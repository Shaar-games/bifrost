...
# is SKIPPED. A 400 (a request we built wrong) or a 500 (emulation returned no
# decision) is our defect and FAILS.
...
def _post_raw(model_id, questions):
...
def _fetch_decision(model_id):
    ...
    resp, latency_ms = _post_raw(model_id, _MATRIX_QUESTIONS)
    ...
    if provider in _SPLIT_PROVIDERS and len(_MATRIX_QUESTIONS) > 1:
        return _fetch_decision_split(model_id, latency_ms)
    return {"status": resp.status_code, "answers": None, "latency_ms": latency_ms, "tokens": None, "cost": "-", "text": resp.text, "split": False}


def _fetch_decision_split(model_id, batched_latency_ms):
    ...
    for name, q in _MATRIX_QUESTIONS.items():
        resp, latency_ms = _post_raw(model_id, {name: q})
...
class TestDecisionEmulationMatrix:
    ...
    def test_provider_model_matches_jev(self, provider, model, jev_reference):
        ...
        try:
            ...
            _MATRIX_ROWS.append({"model_id": model_id, "status": "SKIP", "agree": 0, "total": total, "latency_ms": None, "tokens": None, "cost": "-", "detail": f"request error: {exc}"})
            pytest.skip(f"{model_id}: request error: {exc}")
