...
// validateRoutingFallbacks ensures each fallback names a known provider, and that a pinned key_id
// comes with one. An empty provider covers both the empty string and an unknown provider prefix,
// since schemas.ParseModelString yields "" for each during decoding.
