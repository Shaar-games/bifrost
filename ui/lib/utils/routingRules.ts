...
import { RoutingFallback, RoutingFallbackWire } from "@/lib/types/routingRules";
...
/**
 * Normalizes a fallback to its object form. Rules saved before per-fallback key pinning, and any
 * fallback with no key pinned, come back from the API as a "provider/model" string.
 */
export function normalizeFallback(fallback: RoutingFallbackWire): RoutingFallback {
	...
	return {
		provider: fallback.slice(0, separator),
		model: fallback.slice(separator + 1),
		key_id: "",
	};
}

/**
 * Renders a fallback back onto the wire, as the legacy string unless it pins a key. Sending the
 * object form for an unpinned fallback would change the rule's config hash on every save.
 */
export function denormalizeFallback(fallback: RoutingFallback): RoutingFallbackWire {
	...
	// The slash distinguishes a provider-only fallback from a bare model name.
